    ...
    <nav className="fixed top-0 left-0 right-0 z-50 border-b border-border bg-background/80 backdrop-blur-xl">
      <div className="container mx-auto flex h-14 items-center justify-between px-4">
        ...
        <div className="hidden md:flex items-center gap-1">
          {links.map(({ to, label }) => (
            <Link key={to} to={to} onMouseEnter={() => preloadRoute(to)} onFocus={() => preloadRoute(to)} className={`rounded-md px-3 py-1.5 text-sm font-medium ${isActive(to) ? "bg-primary/10 text-primary" : "text-muted-foreground hover:text-foreground"}`}>
              {label}
            </Link>
          ))}
          {user && (
            <Link to="/inbox" onMouseEnter={() => preloadRoute("/inbox")} onFocus={() => preloadRoute("/inbox")} className={`rounded-md px-2 py-1.5 text-sm font-medium flex items-center gap-1 ${isActive("/inbox") ? "bg-primary/10 text-primary" : "text-muted-foreground hover:text-foreground"}`}>
          ...
          {user && (
            <div className="relative" ref={panelRef}>
              <button onClick={() => setShowNotifs((v) => !v)} className="relative rounded-md px-2 py-1.5 text-muted-foreground hover:text-foreground transition-colors">
              ...
              {showNotifs && (
                <div className="absolute right-0 top-full mt-2 w-80 rounded-xl border border-border bg-card shadow-xl overflow-hidden z-50">
                  <div className="flex items-center justify-between px-4 py-2.5 border-b border-border">
                    ...
                    {unreadCount > 0 && <button onClick={handleMarkAll} className="text-[10px] text-primary hover:underline">Прочитать все</button>}
                  </div>
                  <div className="max-h-72 overflow-y-auto">
                    {notifications.length === 0 ? (
                      <p className="text-sm text-muted-foreground text-center py-6">Нет уведомлений</p>
                    ) : notifications.slice(0, 20).map((n) => (
                      <button key={n.id} onClick={() => handleMarkRead(n.id)} className={`w-full text-left px-4 py-2.5 border-b border-border/50 transition-colors hover:bg-muted/50 ${n.is_read ? "opacity-50" : ""}`}>
          ...
          {user ? (
            <>
              <Link to="/profile" onMouseEnter={() => preloadRoute("/profile")} onFocus={() => preloadRoute("/profile")} className={`rounded-md px-3 py-1.5 text-sm font-medium ${isActive("/profile") ? "bg-primary/10 text-primary" : "text-muted-foreground hover:text-foreground"}`}>
                Профиль
              </Link>
              <button onClick={handleSignOut} className="rounded-md px-2 py-1.5 text-muted-foreground hover:text-foreground transition-colors" title="Выйти">
                <LogOut className="h-4 w-4" />
              </button>
            </>
          ) : (
            <Link to="/login" className="rounded-md px-3 py-1.5 text-sm font-medium text-muted-foreground hover:text-foreground flex items-center gap-1">
